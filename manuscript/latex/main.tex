...
\textit{rbohD}   & Root  & 280 & --  & -- \\
\textit{rbohD}   & Shoot & 33  & --  & -- \\
\textit{cax2-2}  & Root  & 5   & --  & -- \\
\textit{cax2-2}  & Shoot & 3   & --  & -- \\
\botrule
\end{tabular}
\par\smallskip
{\footnotesize Induced/repressed splits are given for Col-0 in the source; full
per-genotype splits are in \texttt{results/tables/deseq2/apex05\_deseq2\_DEG\_counts.csv}.}
\end{table}

\begin{figure}[htbp]
\centering
\fbox{\parbox[c][4.5cm][c]{0.75\textwidth}{\centering\textit{[Figure 2 placeholder.
Render the DEG-count panel from}
\texttt{results/tables/deseq2/apex05\_deseq2\_DEG\_counts.csv} \textit{--- the same
data are tabulated in Table~\ref{tab:deg}.]}}}
\caption{\textbf{Spaceflight differential expression across genotypes.} DESeq2 DEG
counts (induced/repressed) per genotype \(\times\) tissue on the corrected matrix;
Col-0 and \textit{rbohD} respond strongly, \textit{cax2-2} almost not at all.}
\label{fig:2}
\end{figure}
...
\begin{itemize}
  ...
  \item \textbf{RNA-seq \& differential expression.} Reads aligned with HISAT
    \texttt{[TO CONFIRM: versions/genome]}; differential expression with DESeq2
    (pydeseq2), contrast FL vs GC per genotype \(\times\) tissue, flight-referenced;
    DEGs at FDR \(< 0.05\) and \(|\log_2\text{FC}| > 1\).
  \item \textbf{LASSO.} L1-penalised logistic regression on log2-CPM,
  ...
  \item \textbf{Cell-type projection.} PCMDB (Zenodo 10.5281/zenodo.5101271)
    cell-type-specific markers; hypergeometric DEG enrichment + per-sample signature
    scoring; ggPlantmap anatomy.
  \item \textbf{Morphometrics \& integration.} RSML day-4 primary-root traits;
    ...
    transcriptome.
  \item \textbf{Stress resemblance \& pathways.} g:Profiler GO ``response to'' axes
    computed per organ and (via PCMDB markers) per cell type; KEGG pathway membership
    (KEGG REST) and ggkegg maps.
...
KEGG; g:Profiler; Plant Reactome (Gramene). Sequencing archive and Zenodo DOI:
\texttt{[TO CONFIRM]}.
...
\bmhead{References}
\texttt{[TO CONFIRM]} --- the source manuscript has no citations yet. Add entries to
\texttt{references.bib}, insert \texttt{\textbackslash cite\{\}} at the
\texttt{[TO CONFIRM: citations]} points above, then uncomment the
\texttt{\textbackslash bibliography} line below.
