...
\begin{itemize}
  \item \textbf{The problem.} Plants are candidate life-support organisms for
    ...
    transcriptomics (GeneLab / OSDR corpus) established broad reprogramming
    \cite{paul2013}; the causal signalling nodes remain debated.
  \item \textbf{Candidate pathways.} Calcium signalling --- including vacuolar
    Ca\textsuperscript{2+}/H\textsuperscript{+} exchange by \textit{CAX2}
    \cite{shigaki2003,pittman2005} --- and reactive-oxygen production by the NADPH
    oxidase \textit{RBOHD} \cite{torres2002,miller2009} are central to plant
    environmental sensing and proposed as early relays of the spaceflight response.
    APEX-05 tested their contribution by flying \textit{cax2-2} and \textit{rbohD}
    against Col-0. \texttt{[TO CONFIRM: allele line identifiers.]}
